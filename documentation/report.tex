...
Having obtained the best value for K, a new KNN model was trained specifically with this hyperparameter value to show evaluation metrics on the best version of KNN obtainable on this dataset. The obtained performance metrics were the ones in Table [\ref{tab:knnPerformance}].The confusion matrix of KNN with $n\_neighbors = 37$ can be seen in Figure [\ref{fig:confusionMatrixKNN}].

\begin{table}[H]
    \centering
    \caption{KNN Performance Scores}
    \begin{tabular}{lcccc}
        \toprule
        \textbf{F1-Score} & \textbf{Accuracy} & \textbf{Precision} & \textbf{Recall} \\
        \midrule
...
With the best hyperparameter configuration found by the grid search being: $max\_depth = 20$,  $min\_samples\_leaf = 3$, $min\_samples\_split = 2$, $n\_estimators = 150$. The specific optimally configurated model, when trained in isolation, produced the following evaluation metrics can be seen in Table [\ref{tab:rfcPerformance}]. The confusion matrix of RFC with $n\_estimators = 150$ can be seen in Figure [\ref{fig:confusionMatrixRFC}].

\begin{table}[H]
    \centering
    \caption{RFC Performance Scores}
    \begin{tabular}{lcccc}
        \toprule
        \textbf{F1-Score} & \textbf{Accuracy} & \textbf{Precision} & \textbf{Recall} \\
        \midrule
...
It is apparent the best performing mode was $'rbf'$, which is the expected result: this indirectly confirms that the data is not linearly separable and therefore a linear kernel mode could have never performed well enough to surpass its competitors. RBF has the highest performance because it can sort of bend the separation line around data, better fitting the separation and achieving a higher precision and accuracy.\\The statistics that follow are the ones obtained by training a SVM with kernel mode 'rbf' and with $C = 10$ and be seen in Table [\ref{tab:smvPerformance}]. The confusion matrix of said configuration can be seen in Figure [\ref{fig:confusionMatrixSVM}].

\begin{table}[H]
    \centering
    \caption{SVM Performance Scores}
    \begin{tabular}{lcccc}
        \toprule
        \textbf{F1-Score} & \textbf{Accuracy} & \textbf{Precision} & \textbf{Recall} \\
        \midrule
...
\begin{table}[htbp]
    ...
    \begin{tabular}{lcccc}
        ...
        \textbf{Random Forest} & \textbf{0.8583} & \textbf{0.8887} & \textbf{0.8594} & \textbf{0.8565} \\
