...
\begin{abstract}
\noindent
This report presents a \textbf{comprehensive analysis} of the Star Classification dataset, with the aim of comparing the performance of multiple supervised and unsupervised models.
The study investigates how data preprocessing \textit{heavily impacts} model outcomes and explores how different techniques can be combined to leverage their respective strengths.\\Four distinct supervised learning models are evaluated: \textbf{K-Nearest Neighbors (KNN)}, \textbf{Random Forest}, \textbf{Support Vector Machines (SVM)}, and \textbf{Logistic Regression}.
Additionally, unsupervised learning techniques, specifically \textbf{K-Means} and \textbf{Gaussian Mixture Models (GMM)}, are employed to explore the \textit{intrinsic topological nature} of the astronomical data.
...
The classification of celestial objects is a \textbf{fundamental task} in modern astrophysics.
...
The rationale behind this work is to compare the efficacy of \textit{distance-based}, \textit{ensemble}, and \textit{linear} models in handling astronomical data, which is often characterized by \textbf{high dimensionality}, considerable size and high levels of noise.
...
It has evolved from manual inspection to \textbf{automatic pipelines} capable of processing and handling data of orders of magnitude that are incomparable to those historically analyzed by hand.
Current state-of-the-art methods generally fall into two categories depending on the input data format: \textit{ensemble methods} for tabular photometric data and \textit{Deep Learning architectures} for raw spectral or time-series data.\\For datasets consisting of extracted features (magnitudes, colors, redshift), much like the dataset used in this project, \textbf{Gradient Boosted Decision Trees (GBDTs)} are currently considered the \textit{gold standard}, which is one of the reasons why Random Forest Classifier (RFC) was considered as a viable choice and why it performed quite well in this task.
While tabular data with few selected features is efficient, the highest level of accuracy is obtained through the use of "raw" data, these being mathematical data like \textit{1D-Spectra} (a combination of intensity and wavelength) or \textit{light curves} (brightness over time).
...
The \textbf{primary objective} of this project is to develop a \textit{robust machine learning pipeline} for celestial object classification and to find out which of the tested models best fits the task at hand.
...
The single trainings of the various methods were developed separately and then brought together in one single train cycle, to \textbf{standardize} all of them and put all models in the same starting conditions, distribution and random seed generation.
...
The ID columns were dropped since they do not provide real information and would very likely introduce \textit{biases} or \textit{noise} if kept.
...
First, data coming from \textit{sensor malfunction} was manually deleted (e.g., removing rows where $u = -9999$), then outliers were removed based on \textbf{valid photometric ranges}.
...
Keeping categorical, string-like features might have the model learn \textit{inexistent correlations} based on the length of the word or specific letters (e.g., "words starting with 'A' are usually class 1").
Encoding to a simple ID ($1, 2, 3$) removes the "textual" features entirely, leaving only the \textbf{category identity}.
\subsubsection{Feature Engineering}
The features had the following distributions:
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{Figure_1.png}
    \caption{\textbf{Feature Distribution}: This plot visualizes how each of the post-preprocessing features is distributed inside each class.
The most interesting feature is \textit{redshift}, as can be seen by its peculiar distribution with respect to the other ones.}
...
\begin{figure}[H]
    ...
    \caption{\textbf{Initial Correlation Matrix}: The initial correlation analysis.
It is noticeable how many features are \textbf{highly correlated} with each other.
This denotes a need to act on them, deleting the highly correlated ones, which in this analysis were considered the ones having a correlation index higher than 0.9.}
...
\begin{figure}[H]
    ...
    \caption{\textbf{Final Correlation Matrix}: The final correlation analysis.
Features are now much less correlated and appear to be \textbf{distinct enough} to be more than suitable for the following analysis steps.}
...
\label{sec:results}
The dataset was split into \textbf{80\% training and 20\% testing sets}.
...
\subsection{The Redshift Feature}
 An important observation needs to be made regarding the \textit{redshift} feature: the \textbf{influence} that it has on the performance of the models.
...
This feature is heavily tied to the \textit{Doppler Effect} and how light gets stretched by gravity, cosmological stretch and distance.
...
a \textbf{bigger challenge}, it was decided to also drop \textit{redshift} completely from the features included in the training and testing dataset.
By doing so, classification became less trivial and \textbf{solely based on color}.
...
\end{itemize}
 ...
 \begin{itemize}
    ...
    \item \textbf{r\_i:} $r - i$ (\textbf{Red-Near Infrared Index}: Useful for detecting cooler, redder objects 
like M-dwarfs)
    \item \textbf{i\_z:} $i - z$ (\textbf{Near Infrared-Infrared Index}: Helps distinguish high-redshift galaxies)
 \end{itemize}
 Introducing synthetic features means models now have \textbf{compound attributes} with real-world meaning they could use during training and evaluation.
\subsection{Pipeline Training}
Initially, data was scaled once after preprocessing and then given to all models.
\textbf{Mistakenly}, it was not taken into account that when \textbf{cross\_val\_score} splits \textbf{X\_train} into internal folds, the validation fold has already influenced the scaler (mean/variance) used on the training fold.
This \textit{"peeking"} can lead to overly optimistic scores.\\By adding a pipeline in training loops, this mistake was easily fixed and \textbf{data leakage} was prevented by scaling the entire training set before the cross-validation loop.\\It is to note that this observation solely refers to the Supervised Learning phase.
...
KNN is a simple, instance-based algorithm that classifies a new data point by looking at the majority class of its 'k' closest neighbors in the feature space.
It relies on the assumption that similar data points exist in \textit{close proximity} to each other.
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{Figure_4.png}
    \caption{\textbf{K in KNN}: Graph shows how performance (F1-score) grows to a highpoint at $BestK = 37$, where it starts decreasing.
...
RFC is an \textbf{ensemble learning method} that builds multiple decision trees during training and merges them to get a more accurate and stable prediction.
By taking the majority vote from many individual trees, it corrects for the tendency of single decision trees to \textit{overfit} the training data.
\begin{figure}[H]
    \centering
    ...
    \label{fig:correlationMatrix2}
\end{figure}
In this case, there seems to be an \textit{asymptotic behavior}: even though F1-Score increases, it does so ever so slowly, while training time and load increases with an inverse proportion.
...
SVM is a powerful algorithm that finds the \textbf{optimal hyperplane} (boundary) that best separates different classes in an N-dimensional 

space. It aims to \textit{maximize the margin}, that is the distance between the hyperplane and the nearest data points from each class.
...
\begin{figure}[H]
    ...
    \caption{\textbf{Performance of SVM}: Graph shows a comparison between kernel modes and how well they performed cycling through the $C$ values.
It is apparent the best performing mode was $'rbf'$.}
    \label{fig:correlationMatrix2}
\end{figure}
The graph shows the best kernel mode was 'rbf', which is the expected result: this indirectly confirms that the data is \textit{not linearly separable} and therefore a linear kernel mode could have never performed well enough to surpass its competitors.
RBF has the highest performance because it can \textbf{"bend" the separation line} around data, better fitting the separation and achieving a higher precision and accuracy.\\The statistics that follow are the ones obtained by training a SVM with kernel mode 'rbf' and with $C = 100$:
...
\begin{figure}[H]
    ...
    \caption{\textbf{Confusion Matrix}: This matrix shows how good the SVM model was 
at categorizing objects.}
    \label{fig:confusionMatrixSVM}
\end{figure}


\subsubsection{Logistic Regression}
Logistic Regression is a supervised learning algorithm that applies the $'sigmoid'$ (logistic) function to a linear combination of features, mapping the output to a probability between 0 and 1: it makes predictions based on these probabilities, typically using a threshold to assign data points to specific categories.
It is highly sensitive to feature scaling, which is why a StandardScaler was included within its training pipeline.
...
\begin{figure}[H]
    ...
    \caption{\textbf{Performance of LR}: Graph shows a comparison cycling through the $C$ values.
The highest F1-Score was obtained with $C=10$.}
...
Below is a small table summarizing the results of all models: 
...
This comparison highlights a significant \textit{trade-off}: while dimensionality reduction simplifies the feature space, it does not inherently make the data more "clusterable" in a way that aligns with biological or physical ground truth.
\begin{figure}[H] 
    \centering \includegraphics[width=0.8\textwidth]{Figure_12.png} 
    \caption{\textbf{Clustering Visualization}: Comparison between K-Means and GMM trained on raw scaled features (left) versus PCA-reduced features (right).
The PCA projection visually collapses the variance into distinct regions, though significant overlap remains.} 
...
\item \textbf{Conclusion on Viability}: With Normalized Mutual Info (NMI) values hovering below $0.20$, these features, even when optimized via PCA, are insufficient for unsupervised discovery of star classes.
The data appears to exist on a \textit{continuum} rather than in discrete, well-separated density clusters.
\end{itemize}
...
The clustering analysis provided a crucial \textbf{negative result}.
...
Instead, they likely exist on a continuous manifold (the \textit{stellar locus}) where classes overlap significantly.
This finding underscores the necessity of labeled data (Supervised Learning) for this specific astronomical task.
...
Future iterations of this work could explore:
\begin{enumerate}
    \item \textbf{Hierarchical Classification:} Separating Stars from Extragalactic objects first, then classifying Galaxies vs. Quasars.
\item \textbf{Deep Learning on Raw Spectra:} Utilizing 1D Convolutional Neural Networks (CNNs) on the raw spectral data rather than extracted tabular features to capture absorption lines invisible to standard photometry.
\end{enumerate}

In conclusion, while "blind" photometric classification is viable using non-linear ensemble or instance-based methods, the intrinsic topology of the data prevents simple linear separation or unsupervised clustering from achieving high-fidelity results.
