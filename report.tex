...
\begin{figure}[H]
    ...
    \label{fig:correlationMatrix2}
\end{figure}
Having obtained the best value for K, a new KNN model was trained specifically with this hyperparameter value to show evalutaion metrics on the best version of KNN obtainable on this dataset. These were the obtained results:
\begin{itemize}
...
Initially, all kernel instances were tested, namely $['linear', 'poly', 'rbf', 'sigmoid', 'precomputed']$, each with hyperparameter C ranging in $[0.001, 0.01, 0.1, 1, 10, 100]$. Results showed the best performing, even if with poor performance, were 'linear' and 'rbf' kernels.

\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{Figure_5.png}
    \caption{\textbf{n\_neighbours in RFC}: Graph shows how performance (F1-score) grows. In this case, there seems to be an asyntotic behaviour: even though the score increases, it does so ever so slowly, while training time and load increases with an inverse proportion. Taking both information into account, a good balance between \textbf{performance} and \textbf{training time} is achieved with $n\_estimators = TEMP$}
    \label{fig:correlationMatrix2}
\end{figure}
Having obtained the best value for K, a new KNN model was trained specifically with this hyperparameter value to show evalutaion metrics on the best version of KNN obtainable on this dataset. These were the obtained results:
\begin{itemize}
...
\begin{table}[H]
    ...
    \begin{tabular}{lcccc}
        ...
        \textbf{Model} & \textbf{Accuracy} & \textbf{Precision (Macro)} & \textbf{Recall (Macro)} & \textbf{F1 (Macro)} \\
        \midrule
        KNN ($k=15$) & 0.8678 & 0.8402 & 0.8229 & 0.8324 \\
        Random Forest($n\_neighb = 50$) & 0.00 & 0.00 & 0.00 & 0.00 \\
        SVM (RBF) & 0.00 & 0.00 & 0.00 & 0.00 \\
        Logistic Regression & 0.00 & 0.00 & 0.00 & 0.00 \\
        \bottomrule
    \end{tabular}
