...
The following supervised models were implemented using \textbf{5-Fold Cross-Validation} \cite{lectures}:
\begin{itemize}
...
RFC models avoids overfitting data extremely well by taking the majority vote from multiple individual trees, therefore correcting such tendency of single decision trees: by doing so, it ensures a more stable prediction \cite{geron}. Training loop performance is displayed in Figure [\ref{fig:RFCtraining}].
