...
\hypersetup{
    ...
    citecolor=blue,
    urlcolor=blue,
    pdftitle={Star Classification, Regression and Clustering: A Comparative Analysis},
...
\title{\textbf{Star Classification, Regression and Clustering: A Comparative Analysis} \\
...
\begin{itemize}
    ...
    \item \textbf{Linear-Model Set ($X_{linear}$):} Drops the raw magnitudes and retains \textbf{only} the synthetic color indices to reduce multicollinearity (used for KNN, SVM, LR, and clustering).
\end{itemize}

This decision was taken because while models like KNN, SVM and LR and clustering approaches see their performance hindered by correlated features, and in their case those need to be removed, RFC is effectively immune to such an even and therefore can benefit from both keeping the original features and introducing new information on top of it.
...
\begin{itemize}
    ...
    \item \textbf{Logistic Regression:} Trained on the $X_{linear}$ dataset with `class\_weight='balanced'`. To further address the multicollinearity observed in the feature analysis, the pipeline included a \textbf{PCA} step (preserving 95\% variance) prior to the classifier. The model was tuned for regularization strength $C \in [0.001, 0.01, 0.1, 1, 10, 100]$.
\end{itemize}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=1\textwidth]{img/KNN_performance.png}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.7\textwidth]{img/KNN_confm.png}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.7\textwidth]{img/RFC_confm.png}
...
\begin{figure}[htbp] 
    \centering
    \includegraphics[width=0.8\textwidth]{img/optimized_tree.png}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=1\textwidth]{img/SVM_kernels.png}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.7\textwidth]{img/SVM_confm.png}
...
\begin{itemize}
    \item F1-Score: 0.6116
    \item Accuracy: 0.6562
    \item Precision: 0.6004
    \item Recall: 0.6487
\end{itemize}
The confusion matrix of the Logistic Regression model with $C=1$ is displayed in Figure [\ref{fig:confusionMatrixLR}].

\begin{figure}[htbp]
    \centering
    \includegraphics[width=1\textwidth]{img/LR_performance.png}
    \caption{\textbf{Performance of LR}: Comparison cycling through the $C$ values.}
    \label{fig:LRtraining}
\end{figure}

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.7\textwidth]{img/LR_confm.png}
...
\begin{table}[htbp]
    ...
    \begin{tabular}{lcccc}
        ...
        \midrule
        K-Means (raw) & 0.1068 & 0.1904 & 0.3516 \\
        K-Means (PCA) & 0.1129 & 0.1919 & 0.4333 \\
        GMM (raw) & 0.0990 & 0.1466 & 0.1481 \\
        GMM (PCA) & 0.0652 & 0.1302 & 0.3816 \\
        \bottomrule
    \end{tabular}
